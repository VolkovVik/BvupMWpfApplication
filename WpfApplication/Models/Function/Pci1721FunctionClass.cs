...
namespace WpfApplication.Models.Function {
    internal class Pci1721FunctionClass  {
        /// <summary>
        /// Проверка допустимости данных сигнала
        /// </summary>
        /// <param name="signal"></param>
        private void Check( PlaceSignalDescription signal ) {
            const string protocol    = Protocol.Pci1721;
            var          max_device  = App.TaskManager.Port1721.MaxDevice;
            var          max_channel = App.TaskManager.Port1721.MaxChannel;

            // Проверка инициализации интерфейсной платы 
            if ( null == App.TaskManager.Port1721 || !App.TaskManager.Port1721.IsInit ) {
                throw new Exception< DeviceNotInitializedExceptionArgs >( new DeviceNotInitializedExceptionArgs(),
                    "Ошибка! Плата PCI-1721 не инициализирована" );
            }
        ...
        public void Set( PlaceSignalDescription signal, double value ) {
            Check( signal );
            // Выдача аналогового сигнала с интерфейсной платы PCI-1721
            App.TaskManager.Port1721.Write( signal.Device ?? 0, signal.Channel ?? 0, value );
        }
    }
}